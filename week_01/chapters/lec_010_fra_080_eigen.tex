...
Определение. Если для действия $\LL: \R^n \to \R^n$ найдётся такой вектор $v$, 
что $\LL v=\lambda \cdot v$, где $\lambda \in \R$, то:
  \begin{itemize}
    \item вектор $v$ называется \alert{собственным};
    \item число $\lambda$ называется \alert{собственным}.
  \end{itemize} 
...
\begin{frame}{Собственные вектора: растягивание вдоль осей}
...
\item Собственные векторы с $\lambda = 2$
...
\item Собственные векторы с $\lambda = -3$
...
\begin{frame}{Собственные вектора: перестановка компонент}
...
\begin{itemize}[<+->]
    \item Не у каждого линейного оператора есть собственные векторы!
    \item По определению, $\LL: \R^n \to \R^n$
    \item 
    Исходный оператор $R: \R^2 \to \R^2$, поворот на $30^{\circ}$ против часовой стрелки.
    \item Собственных векторов (и чисел) нет!
\end{itemize}
