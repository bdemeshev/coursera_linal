\documentclass[14 pt,xcolor=dvipsnames]{beamer}

\input{linal_preamble.tex}
\usepackage{physics}
\newcommand{\R}{\mathbb{R}}
\newcommand{\Rot}{\mathrm{R}}
\newcommand{\HH}{\mathrm{H}}
\newcommand{\Id}{\mathrm{I}}


\usepackage[outline]{contour}




\usepackage{pgfplots}
\pgfplotsset{compat=newest}

\usepackage{tikz}
\usetikzlibrary{calc}
\usetikzlibrary{quotes,angles}
\usetikzlibrary{arrows}
\usetikzlibrary{arrows.meta}
\usetikzlibrary{positioning,intersections,decorations.markings}
\usetikzlibrary{patterns}

\usepackage{tkz-euclide} 

\newcommand{\grid}{\draw[color=gray,step=1.0,dotted] (-2.1,-2.1) grid (9.6,6.1)}

\newcommand{\ba}{\symbf{a}}
\newcommand{\be}{\symbf{e}}
\newcommand{\bb}{\symbf{b}}
\newcommand{\bc}{\symbf{c}}
\newcommand{\bd}{\symbf{d}}
\newcommand{\bx}{\symbf{x}}
\newcommand{\bv}{\symbf{v}}
\newcommand{\bzero}{\symbf{0}}


\DeclareMathOperator{\Lin}{Span}

\DeclareMathOperator{\Span}{Span}
\DeclareMathOperator{\LL}{L}

%\tikzset{>=latex}

\colorlet{veca}{red}
\colorlet{vecb}{blue}
\colorlet{vecc}{olive}







\begin{document}



% ## Матрица линейного оператора


% Видео 1. Линейная комбинация векторов. Линейно зависимые и независимые наборы векторов.

% Видео 2. Линейная оболочка векторов. Базис линейной оболочки. Размерность линейной оболочки векторов. 

% Видео 3. Общее понятие линейного пространства. Базис линейного пространства. Размерность.

% Видео 4. Множество значений линейного оператора как линейная оболочка. Ранг линейного оператора: поворот, проекция, отражение. 
% След проекции — просто как особый случай (!)

% Видео 5. Матрица как способ записи линейного оператора. 
% Матрицы поворота, проекции, растяжения. Единичная матрица. 

% Видео 6. (доска) Умножение матрицы на вектор как линейная комбинация столбцов. 

% Видео 7. (доска) Умножение матриц как последовательное применение операторов. 

% Видео 8. (доска) Метод Гаусса решения систем. 

% задача про краски?
% или про коров? 101 корова, убираем любую одну и всегда сможем поделить 50 на 50 на стада с равными массами




% \maketitle


\begin{frame} % название лекции


\lecturetitle{Матричная запись}

\end{frame}


\input{chapters/lec_020_fra_010_linear_span.tex}




\end{document}
